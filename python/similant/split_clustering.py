...
def process_clustering(
    ...
    for index in range(1, n_clusters + 1):
        ...
        clusters = _clusters_visualization(
            distance_table, model["clusters"], descriptors, mapping)
    ...
    if not mapping is None:
        ...
        descriptor_data = {}
        for ids in mapping:
            for item in mapping[ids]:
                descriptor_data[item] = tmp[ids]
...
def _clusters_visualization(results, input_clusters, descriptors, mapping=None):
    keys = list(map(lambda x: int(x), list(input_clusters.keys())))

    logging.info("t-SNE ...")
    tsne = TSNE(n_components=2, verbose=0, perplexity=30, n_iter=1000, metric="precomputed", random_state=0)
    manifold_results = tsne.fit_transform(results[keys][:,keys])
    logging.info("t-SNE ... done")

#    logging.info("MDS ...")
#    mds = MDS(n_components=2, verbose=0, dissimilarity="precomputed", random_state=0)
#    manifold_results = mds.fit_transform(results[keys][:,keys])
#    logging.info("MDS ... done")
    ...
    for cid in input_clusters:
        ...
        if not mapping is None:
            parts = list(map(lambda item: mapping[item] if item in mapping else [item], items))
            items = []
            for item in parts:
                items += item
        ...
        if not mapping is None and input_clusters[cid]["id"] in mapping:
            input_clusters[cid]["id"] = mapping[input_clusters[cid]["id"]][0]
        key = descriptors[int(cid)]["id"]
        if not mapping is None and key in mapping:
            key = mapping[key][0]
        output[key] = input_clusters[cid]
    return output
